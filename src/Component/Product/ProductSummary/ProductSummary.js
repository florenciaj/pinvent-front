...
import { CALCULATE_CATEGORY, CALCULATE_OUT_OF_STOCK, CALCULATE_STORE_VALUE, selectCategory, selectOutOfStock, selectTotalStoreValue } from '../../../Redux/Feature/Product/ProductSlice';
  ...
  return (
    <div className="product-summary">
      ...
      <div className="info-summary">
        ...
        <Box icon={earningIcon} title={'Total store value'} quantity={'0'} amount={`$${formatNumbers(totalStoreValue.toFixed(2))}`} backgroundColor='card2'></Box>
        <Box icon={outOfStockIcon} title={'Out of stock'} quantity={'0'} amount={totalOutOfStock} backgroundColor='card3'></Box>
        <Box icon={categoryIcon} title={'Total categories'} quantity={'0'} amount={totalCategories} backgroundColor='card4'></Box>
      </div>
    </div>
  )
}
